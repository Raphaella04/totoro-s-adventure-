...
class Menu:
    ...
    def create_buttons(self):
        ...
        self.stone_button_rect = generic_button_rect.move(0,self.rect.width/ 2).inflate(-button_margin,-button_margin)
        ...
        Button(self.stone_button_rect, self.buttons, self.menu_surfs['stone'])
        
    def click(self,mouse_pos,mouse_button):
        for sprite in self.buttons:
            if sprite.rect.collidepoint(mouse_pos):
                if mouse_button[1]: #midle mouse click
                    # sprite does not have alt items it's true
                    if sprite.items['alt']:
    ...
    def highlight_indicator(self, index):
        ...
        if EDITOR_DATA[index]['menu'] == 'stone':
            pygame.draw.rect(self.display_surface, BUTTON_LINE_COLOR , self.stone_button_rect.inflate(4,4),5,4)
...
class Button(pygame.sprite.Sprite):
    ...
    def get_id(self):
        #going through the dictionary to select items or items alt
        return self.items['main' if self.main_active else 'alt'][self.index][0] 

    def switch(self):
         self.index += 1
         # the index will be set to zero
         #if the index is greater than length of the main items or alt items
